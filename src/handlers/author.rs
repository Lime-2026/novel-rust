...
pub(crate) async fn get_author(
    ...
    if !file_exists(format!("templates/{}/author.html", CONFIG.theme_dir)) {
    ...
    let rows = utils::redis::conn::get_cache_rows(
        format!("SELECT {filed} FROM {table}article_article WHERE {where} AND author = ? ORDER BY lastupdate DESC;", filed = CONFIG.get_field(), table = CONFIG.prefix, where = CONFIG.get_where()),
        url,
        CONFIG.cache.other as u64,
    ...
    let template_path = format!("{}/author.html", CONFIG.theme_dir);
